...
## 15. SSO Setup (Phase 3 — optional, khi ready test OAuth)

### Prerequisites

- iNET GitLab OAuth app **registered** với iNET GitLab admin:
  - **Application name:** OneMCP Staging (hoặc OneMCP Prod)
  - **Redirect URI:** `https://<your-staging-ip>/api/auth/gitlab/callback`
  - **Scopes:** `read_user` (chỉ đọc email)
  - **Confidential:** Yes
  - **Result:** Lấy Application ID + Application Secret (show once)

- TLS cert phải cover IP hoặc domain (OAuth callback phải HTTPS). Nếu chưa, regenerate:
  ```bash
  cd /opt/onemcp
  bash ops/nginx/generate-self-signed-tls.sh <your-staging-domain-or-ip>
  docker compose restart nginx
  ```

### 15.1 Configure .env cho SSO

Thêm vào `.env`:

```env
# OAuth
AUTH_MODE=gitlab-sso
NEXT_PUBLIC_AUTH_MODE=gitlab-sso

# GitLab OAuth app
GITLAB_SSO_BASE_URL=https://gitlab.inet.vn
GITLAB_OAUTH_CLIENT_ID=<Application-ID-from-GitLab-admin>
GITLAB_OAUTH_CLIENT_SECRET=<Application-Secret-from-GitLab-admin>
GITLAB_OAUTH_REDIRECT_URI=https://<your-staging-ip>/api/auth/gitlab/callback
...
# OAuth state nonce (random hex, 32 chars)
OAUTH_STATE_NONCE_KEY=<random-hex-32-chars>

# Roles bootstrap (email-based)
MAINTAINER_USERNAMES=admin@inet.vn,devlead@inet.vn
ADMIN_USERNAMES=admin@inet.vn

# Access mode (open = no IP guard after SSO)
ACCESS_MODE=open
```

**Generate random keys:**
```bash
openssl rand -hex 16  # OAUTH_STATE_NONCE_KEY
```

### 15.2 Rebuild + restart

```bash
cd /opt/onemcp

# Rebuild backend + portal (NEXT_PUBLIC_AUTH_MODE is build-time)
docker compose build backend portal
...
### 15.3 Verify OAuth flow

**From staging VPS (localhost test):**

```bash
# 1. Check backend health includes auth mode
curl -sk https://localhost/api/health | jq .mode
# Expect: "gitlab-sso"

# 2. Check portal redirects /login
curl -sk https://localhost/ -L | grep -i "sign in with\|gitlab"
# Expect: HTML contains "Sign in with iNET GitLab" button

# 3. Simulate callback (需要 real GitLab account)
# Manually: browser https://<staging-ip>/ → click button → GitLab authorize → callback
```

**From your machine (browser):**

1. Open `https://<staging-ip>/`
2. Should redirect `/login`
3. Click "Sign in with iNET GitLab"
4. GitLab consent screen → Authorize
5. Redirect back to `/` with session cookie
6. Check browser dev tools (F12 → Application → Cookies) — should see `session=...`

**If stuck at GitLab consent:**
- Check staging firewall allows outbound 443 to `gitlab.inet.vn`
- Check redirect URI matches exactly in GitLab OAuth app config

**If redirect fails / 5xx:**
```bash
docker compose logs backend | tail -30 | grep -i "auth\|error"
```

### 15.4 Test legacy auth (trust-header rollback)

Verify fallback still works:

```bash
# Set env back to trust-header
sudo sed -i 's/^AUTH_MODE=.*/AUTH_MODE=trust-header/' /opt/onemcp/.env
sudo sed -i 's/^NEXT_PUBLIC_AUTH_MODE=.*/NEXT_PUBLIC_AUTH_MODE=trust-header/' /opt/onemcp/.env
...
# Test legacy path
curl -sk https://localhost/api/artifacts -H "X-Onemcp-User: testuser"
# Expect: 200 (or JSON, depending on data)
```
